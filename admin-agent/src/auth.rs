...
impl AuthState {
    pub fn from_env() -> Self {
        Self {
            ...
            jwt_refresh_secret: env::var("ADMIN_JWT_REFRESH_SECRET")
                .expect("falta ADMIN_JWT_REFRESH_SECRET"),
            admin_username: env::var("ADMIN_USERNAME").expect("falta ADMIN_USERNAME"),
            admin_password_hash: env::var("ADMIN_PASSWORD_HASH")
                .expect("falta ADMIN_PASSWORD_HASH"),
...
pub async fn require_auth(
    ...
    .map_err(|e| {
        // 👇 ESTE ES EL CAMBIO CLAVE: Logueamos el error real en el servidor
        tracing::warn!("🚫 Fallo de autenticación JWT: {:?}", e);
        StatusCode::UNAUTHORIZED
    })?;

    if decoded.claims.token_type != "access" {
        tracing::warn!("🚫 Tipo de token incorrecto: se esperaba 'access'");
        return Err(StatusCode::UNAUTHORIZED);
    }
